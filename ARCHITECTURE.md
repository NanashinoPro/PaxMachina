...
*   **国家首脳・閣僚推論（マルチエージェント） (`gemini-2.5-pro`)**: これまでの首脳単独の意思決定から、三閣僚（外務、防衛、経済内務）が並列（ThreadPoolExecutor）で各省庁の利害と状況を基に政策を起案し、首脳（大統領）が全ての草案をレビュー・採択・修正して最終決定を下すパイプラインへと進化。高度な論理的推論と予算配分の調整力が要求されるコア・エンジン。
