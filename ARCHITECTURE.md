...
*   **同盟 (Alliance) / 戦争 (War) / 貿易協定 (Trade Agreement) / 制裁 (Sanction):** 国家間で結ばれた外交的ステータス。
...
軍事保有ストック ($M$) の維持コストとなる疲弊係数 $\alpha_{mil}$ は、軍事負担率（$Military / GDP$）の二乗に比例して動的に跳ね上がる。
$$ \alpha_{mil} = \min\left(0.20,\ 0.03 + \left(\frac{Military}{\max(1, GDP)} \times 2.0\right)^2\right) $$
次ターンの軍事力は以下のように算出され、過剰な軍拡は維持費の増大により自壊する。
...
*   対象国へのデバフ = $\min(10.0\%,\  2.0\% \times \frac{GDP_{imposer}}{\max(1, GDP_{target})})$ （乗算、最大10%低下）
*   発動国へのデバフ = 常に $1.0\%$ の経済活動低下（乗算 $0.99$）
...
*   **産業空洞化ペナルティ**：3ターン連続で赤字を記録し続けた場合、長期的な失業増大を反映し、赤字国の支持率に最大 $-5.0\%$ の重い継続ペナルティが付与される。
...
#### 民主主義 (Democracy)
*   支持率が `CRITICAL_APPROVAL (15.0%)` 未満になると、直ちに政府機能が麻痺して政権が崩壊・交代する。この際、軍事力が半減（$\times 0.5$）、経済力が $30\%$ 毀損（$\times 0.7$）され、新政権の支持率は $40.0\%$ にリセットされる。
*   4年（16ターン）ごとの大統領選挙を迎えた際、落選確率 $= (100 - Approval) / 100$ で合否判定が行われる。

#### 専制主義 (Authoritarian)
*   支持率が $30.0\%$ 未満の場合、毎ターン `Rebellion Risk` が $+5.0\%$ ずつ蓄積する（$30.0\%$ 以上の場合は $-2.0\%$ 減少）。
