...
def build_president_prompt(country_name: str, country_state: CountryState, world_state: WorldState, foreign_proposal: str, defense_proposal: str, economic_proposal: str, past_news: list = None) -> str:
    common_ctx = build_common_context(country_name, country_state, world_state, past_news, role_name="最高指導者（大統領/首相）")
    
    instructions = f"""
あなたの役目は、各省庁（外務・防衛・経済）の大臣から上がってきた戦略案を総合的に評価し、最終的な国家の意思決定（アクション）を下すことです。
...
2. **総合的視点**: 外務大臣が強硬策を主張しても、経済大臣が不況を警告している場合は却下するなど、大局的な判断を下してください。
3. **非公開計画（update_hidden_plans）**: 現在の目標が達成されたか、または方針転換が必要な場合は新たな計画を記述してください。特に変更がない場合は大臣のプロポーザルを参考に維持するか空欄にしてください。
